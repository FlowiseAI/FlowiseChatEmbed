...
const CardWithDeleteOverlay = ({ item, disabled, customization, onDelete }) => {
  ...
  return (
    <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{ position: 'relative', display: 'inline-block' }}>
      <Card
        sx={{
          display: 'inline-flex',
          alignItems: 'center',
...
const ChatMessage = ({ open, chatflowid, isAgentCanvas, isDialog, previews, setPreviews }) => {
  ...
  useEffect(() => {
    if (getAllExecutionsApi.data?.length) {
      ...
      const loadedMessages = getAllExecutionsApi.data.map((execution) => {
        const executionData = typeof execution.executionData === 'string' ? JSON.parse(execution.executionData) : execution.executionData;
        const obj = {
          id: execution.id,
  ...
  const previewDisplay = (item) => {
    if (item.mime.startsWith('image/')) {
      ...
      return (
        <CardWithDeleteOverlay disabled={getInputDisabled()} item={item} customization={customization} onDelete={() => handleDeletePreview(item)} />
      );
    }
  };
        ...
        >
          <Box
            sx={{
              ...
              p: 3,
              backgroundColor: customization.isDarkMode ? darken(theme.palette.background.paper, 0.2) : theme.palette.background.paper,
    ...
    <div onDragEnter={handleDrag}>
      ...
      {isDragActive && (getAllowChatFlowUploads.data?.isImageUploadAllowed || getAllowChatFlowUploads.data?.isRAGFileUploadAllowed) && (
        <Box className="drop-overlay">
          ...
          {[...getAllowChatFlowUploads.data.imgUploadSizeAndTypes, ...getAllowChatFlowUploads.data.fileUploadSizeAndTypes].map((allowed) => {
            return (
              <>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {message.agentFlowExecutedData && Array.isArray(message.agentFlowExecutedData) && message.agentFlowExecutedData.length > 0 && (
                    ...
                    {message.action && (
                      ...
                      >
                        {(message.action.elements || []).map((elem, index) => {
                          return (
                            <>
                              {(elem.type === 'approve-button' && elem.label === 'Yes') || elem.type === 'agentflowv2-approve-button' ? (
                                <Button
                                  sx={{
                                    width: 'max-content',
                                    borderRadius: '20px',
                              ...
                              ) : (elem.type === 'reject-button' && elem.label === 'No') || elem.type === 'agentflowv2-reject-button' ? (
                                <Button
                                  sx={{
                                    width: 'max-content',
                                    borderRadius: '20px',
                      ...
                      <>
                        ...
                        >
                          {isTTSEnabled && (
                            <IconButton
                              size="small"
                              onClick={() => (isTTSPlaying[message.id] ? handleTTSStop(message.id) : handleTTSClick(message.id, message.message))}
                            ...
                            >
                              {isTTSLoading[message.id] ? (
                                ...
                                <IconVolume style={{ width: '20px', height: '20px' }} color={customization.isDarkMode ? 'white' : '#1e88e5'} />
                              )}
                            </IconButton>
                          )}
                          {chatFeedbackStatus && (
            ...
            {recordingNotSupported ? (
              <div className="overlay">
                <div className="browser-not-supporting-audio-recording-box">
                  <Typography variant="body1">To record audio, use modern browsers like Chrome or Firefox that support audio recording.</Typography>
