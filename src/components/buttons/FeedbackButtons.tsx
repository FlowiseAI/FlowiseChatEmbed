...
import { ClipboardIcon, DeleteIcon, ThumbsDownIcon, ThumbsUpIcon } from '../icons';
...
export const RegenerateResponseButton = (props: RatingButtonProps) => {
  return (
    ...
    >
      <Show when={!props.isLoading} fallback={<Spinner class="text-white" />}>
        <DeleteIcon color={props.feedbackColor ?? defaultFeedbackColor} class={'send-icon flex ' + (props.disableIcon ? 'hidden' : '')} />
      </Show>
    </button>
  );
};
